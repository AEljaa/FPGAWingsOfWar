...
#define  NUM_TAPS 28
#define Q_BITS 8
#define SCALE (1 << Q_BITS)

#define FLOAT_TO_FIXED(x) ((int)((x) * SCALE))
#define FIXED_MULT(a, b) ((alt_32)(((alt_64)(a) * (alt_64)(b)) >> Q_BITS))

void floatArrayToFixed(int fixedArray[], float floatArray[], int arraySize) {
    for (int i = 0; i < arraySize; i++) {
        fixedArray[i] = FLOAT_TO_FIXED(floatArray[i]);
    }
}
...
int filter_coefficients[NUM_TAPS];
...
alt_32 filterFIR(alt_32 acc_read) {
    ...
    for (int i = 0; i < NUM_TAPS; i++) {
        filtered_value += FIXED_MULT(filter_state[i], filter_coefficients[i]);
    }
